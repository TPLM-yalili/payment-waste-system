        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg sm:rounded-lg p-6">
                ...
                @if($invoices->every(fn($invoice) => $invoice->status == 'paid'))
                    <!-- Jika semua tagihan sudah dibayar -->
                    <div class="bg-green-50 p-6 rounded-lg shadow-md mb-6">
                        <h4 class="text-xl font-semibold text-green-700">Terima Kasih!</h4>
                        <p class="text-gray-600">Anda telah berhasil membayar semua tagihan sampah bulan ini. Terima kasih atas perhatian Anda!</p>
                    </div>
                @else
                    <!-- Tagihan Bulan Ini -->
                    <div class="mb-6">
                        ...
                        @foreach($invoices as $invoice)
                            <div class="bg-gray-50 p-5 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                                <div class="flex justify-between items-center mb-3">
                                    <p class="text-lg font-medium text-gray-900">Order ID: {{ $invoice->order_id }}</p>
                                    <span
                                        class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($invoice->due_date)->format('d M Y') }}</span>
                                </div>
                                <p class="text-xl font-semibold text-blue-600">Rp
                                    {{ number_format($invoice->amount, 0, ',', '.') }}</p>
                                <p class="text-gray-600">Due Date:
                                    {{ \Carbon\Carbon::parse($invoice->due_date)->format('d M Y') }}</p>
                                <form action="{{ route('invoice.pay', $invoice->id) }}" method="GET" class="mt-4">
                                    <button type="submit"
